...
class employee_payroll_report(osv.osv_memory):
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        domain1 = []
        if obj.category_id:
            domain1.append('category', '=', obj.category_id.id)
        if obj.bank_id:
                domain1.append(('bank_field', '=', obj.bank_id.id))       
        employee_ids = employee_obj.search(cr , uid, domain1)
        ...
        if employee_ids:
            ...
            if payslip_ids:
                ...
                worksheet.write(3, 3, 'Account Number', style)
                worksheet.write(3, 4, 'Bank Name', style)
                worksheet.write(3, 5, 'Month Days', style)
                worksheet.write(3, 6, 'Paid Days', style)
                worksheet.write(3, 7, 'Over Time', style)
                for l in newlist:
                    print "list", l
                    dic[l['id']] = k
    ...
    #             worksheet.write(4,7,current_user)
                ...
                for emp_id in employee_ids:
                    ...
                    if payslip_id:
                        ...
                        worksheet.write(j, p + 3, emp_obj.acc_number)
                        worksheet.write(j, p + 4, emp_obj.bank_field.name)
                        payslip_brw = payslip_obj.browse(cr, uid, payslip_id)[0]
                        for workDaysLine in payslip_brw.worked_days_line_ids:
                            if workDaysLine.code == 'SALARYDAYS':
                                worksheet.write(j, p + 6, workDaysLine.number_of_days)
                                worksheet.write(j, p + 7, workDaysLine.number_of_hours)
                            if workDaysLine.code == 'MONTHDAYS':
                                worksheet.write(j, p + 5, workDaysLine.number_of_days) 
